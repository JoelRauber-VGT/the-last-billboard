...
export default function BidPage({
    ...
    async function init() {
      ...
      if (outbidParam) {
        ...
        const slot: SlotInfo = {
          ...
          current_bid_eur: data.current_bid_eur as number,
          current_owner_id: data.current_owner_id as string | null,
        };
  ...
  const onSubmit = async (values: BidFormValues) => {
    ...
    try {
      ...
      if (!result.success) {
        setError(result.error || tErrors('sessionFailed'));
        setIsLoading(false);
        return;
      }
            ...
            <FormField
              ...
              render={({ field: { value, onChange, ...fieldProps } }) => (
                <FormItem>
                  ...
                  <FormDescription>{t('form.imageHelp')}</FormDescription>
                  <FormMessage />
                </FormItem>
              )}
            />
